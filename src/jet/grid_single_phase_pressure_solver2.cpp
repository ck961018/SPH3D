...
void buildSingleSystem(FdmMatrix2* A, FdmVector2* b, const Array2<char>& markers,
...
void buildSingleSystem(MatrixCsrD* A, VectorND* x, VectorND* b, const Array2<char>& markers,
                       const FaceCenteredGrid2& input) {
  Size2 size = input.resolution();
  Vector2D invH = 1.0 / input.gridSpacing();
  Vector2D invHSqr = invH * invH;
...
void GridSinglePhasePressureSolver2::solve(const FaceCenteredGrid2& input,
                                           double timeIntervalInSeconds, FaceCenteredGrid2* output,
...
GridBoundaryConditionSolver2Ptr GridSinglePhasePressureSolver2::suggestedBoundaryConditionSolver()
    const {
  return std::make_shared<GridBlockedBoundaryConditionSolver2>();
}

const FdmLinearSystemSolver2Ptr& GridSinglePhasePressureSolver2::linearSystemSolver() const {
...
void GridSinglePhasePressureSolver2::applyPressureGradient(const FaceCenteredGrid2& input,
