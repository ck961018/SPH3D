...
void GridFractionalBoundaryConditionSolver3::constrainVelocity(FaceCenteredGrid3* velocity,
                                                               unsigned int extrapolationDepth) {
  Size3 size = velocity->resolution();
  if (_colliderSdf == nullptr || _colliderSdf->resolution() != size) {
    updateCollider(collider(), size, velocity->gridSpacing(), velocity->origin());
  }
  ...
  extrapolateToRegion(velocity->uConstAccessor(), uMarker, extrapolationDepth, u);
  extrapolateToRegion(velocity->vConstAccessor(), vMarker, extrapolationDepth, v);
  extrapolateToRegion(velocity->wConstAccessor(), wMarker, extrapolationDepth, w);
  ...
  velocity->parallelForEachUIndex([&](size_t i, size_t j, size_t k) {
    ...
    if (isInsideSdf(_colliderSdf->sample(pt))) {
      ...
      if (g.lengthSquared() > 0.0) {
        ...
        Vector3D velt = projectAndApplyFriction(velr, n, collider()->frictionCoefficient());
  ...
  // Transfer results
  u.parallelForEachIndex([&](size_t i, size_t j, size_t k) { u(i, j, k) = uTemp(i, j, k); });
  v.parallelForEachIndex([&](size_t i, size_t j, size_t k) { v(i, j, k) = vTemp(i, j, k); });
  w.parallelForEachIndex([&](size_t i, size_t j, size_t k) { w(i, j, k) = wTemp(i, j, k); });
...
ScalarField3Ptr GridFractionalBoundaryConditionSolver3::colliderSdf() const { return _colliderSdf; }

VectorField3Ptr GridFractionalBoundaryConditionSolver3::colliderVelocityField() const {
  return _colliderVel;
}

void GridFractionalBoundaryConditionSolver3::onColliderUpdated(const Size3& gridSize,
  ...
  if (collider() != nullptr) {
    ...
    _colliderSdf->fill([&](const Vector3D& pt) { return implicitSurface->signedDistance(pt); });

    _colliderVel = CustomVectorField3::builder()
                       .withFunction([&](const Vector3D& x) { return collider()->velocityAt(x); })
