...
double SphSolver3::eosExponent() const { return _eosExponent; }

void SphSolver3::setEosExponent(double newEosExponent) {
  _eosExponent = std::max(newEosExponent, 1.0);
}

double SphSolver3::negativePressureScale() const { return _negativePressureScale; }
...
double SphSolver3::viscosityCoefficient() const { return _viscosityCoefficient; }
...
double SphSolver3::pseudoViscosityCoefficient() const { return _pseudoViscosityCoefficient; }
...
double SphSolver3::speedOfSound() const { return _speedOfSound; }

void SphSolver3::setSpeedOfSound(double newSpeedOfSound) {
  _speedOfSound = std::max(newSpeedOfSound, kEpsilonD);
}

double SphSolver3::timeStepLimitScale() const { return _timeStepLimitScale; }
...
unsigned int SphSolver3::numberOfSubTimeSteps(double timeIntervalInSeconds) const {
  ...
  double timeStepLimitByForce =
      kTimeStepLimitByForceFactor * std::sqrt(kernelRadius * mass / maxForceMagnitude);

  double desiredTimeStep =
      _timeStepLimitScale * std::min(timeStepLimitBySpeed, timeStepLimitByForce);
...
void SphSolver3::onBeginAdvanceTimeStep(double timeStepInSeconds) {
  ...
  JET_INFO << "Building neighbor lists and updating densities took " << timer.durationInSeconds()
           << " seconds";
}
...
void SphSolver3::onEndAdvanceTimeStep(double timeStepInSeconds) {
  ...
  JET_INFO << "Max density: " << maxDensity << " "
           << "Max density / target density ratio: " << maxDensity / particles->targetDensity();
}
...
void SphSolver3::computePressure() {
  ...
  parallelFor(kZeroSize, numberOfParticles, [&](size_t i) {
    p[i] = computePressureFromEos(d[i], targetDensity, eosScale, eosExponent(),
                                  negativePressureScale());
  });
}

void SphSolver3::accumulatePressureForce(const ConstArrayAccessor1<Vector3D>& positions,
...
void SphSolver3::accumulateViscosityForce() {
  ...
  parallelFor(kZeroSize, numberOfParticles, [&](size_t i) {
    ...
    for (size_t j : neighbors) {
      ...
      f[i] += viscosityCoefficient() * massSquared * (v[j] - v[i]) / d[j] *
...
void SphSolver3::computePseudoViscosity(double timeStepInSeconds) {
  ...
  parallelFor(kZeroSize, numberOfParticles,
              [&](size_t i) { v[i] = lerp(v[i], smoothedVelocities[i], factor); });
}

SphSolver3::Builder SphSolver3::builder() { return Builder(); }
