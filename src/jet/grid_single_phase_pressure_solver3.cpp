...
void buildSingleSystem(FdmMatrix3* A, FdmVector3* b, const Array3<char>& markers,
...
void buildSingleSystem(MatrixCsrD* A, VectorND* x, VectorND* b, const Array3<char>& markers,
                       const FaceCenteredGrid3& input) {
  Size3 size = input.resolution();
  Vector3D invH = 1.0 / input.gridSpacing();
  Vector3D invHSqr = invH * invH;
...
void GridSinglePhasePressureSolver3::solve(const FaceCenteredGrid3& input,
                                           double timeIntervalInSeconds, FaceCenteredGrid3* output,
                                           const ScalarField3& boundarySdf,
                                           const VectorField3& boundaryVelocity,
                                           const ScalarField3& fluidSdf, bool useCompressed) {
...
GridBoundaryConditionSolver3Ptr GridSinglePhasePressureSolver3::suggestedBoundaryConditionSolver()
    const {
  return std::make_shared<GridBlockedBoundaryConditionSolver3>();
}

const FdmLinearSystemSolver3Ptr& GridSinglePhasePressureSolver3::linearSystemSolver() const {
...
void GridSinglePhasePressureSolver3::buildMarkers(
    const Size3& size, const std::function<Vector3D(size_t, size_t, size_t)>& pos,
...
void GridSinglePhasePressureSolver3::buildSystem(const FaceCenteredGrid3& input,
  ...
  if (_mgSystemSolver == nullptr) {
    ...
    FdmMgUtils3::resizeArrayWithFinest(size, maxLevels, &_mgSystem.A.levels);
    FdmMgUtils3::resizeArrayWithFinest(size, maxLevels, &_mgSystem.x.levels);
    FdmMgUtils3::resizeArrayWithFinest(size, maxLevels, &_mgSystem.b.levels);
  ...
  if (_mgSystemSolver == nullptr) {
    if (useCompressed) {
      buildSingleSystem(&_compSystem.A, &_compSystem.x, &_compSystem.b, _markers[0], *finer);
    } else {
      buildSingleSystem(&_system.A, &_system.b, _markers[0], *finer);
    }
  } else {
    buildSingleSystem(&_mgSystem.A.levels.front(), &_mgSystem.b.levels.front(), _markers[0],
                      *finer);
  }
  ...
  for (size_t l = 1; l < numLevels; ++l) {
    ...
    buildSingleSystem(&_mgSystem.A.levels[l], &_mgSystem.b.levels[l], _markers[l], coarser);

    finer = &coarser;
  }
}

void GridSinglePhasePressureSolver3::applyPressureGradient(const FaceCenteredGrid3& input,
