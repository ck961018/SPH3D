...
GridSystemData2::GridSystemData2() : GridSystemData2({0, 0}, {1, 1}, {0, 0}) {}

GridSystemData2::GridSystemData2(const Size2& resolution, const Vector2D& gridSpacing,
...
void GridSystemData2::resize(const Size2& resolution, const Vector2D& gridSpacing,
                             const Vector2D& origin) {
...
size_t GridSystemData2::addAdvectableScalarData(const ScalarGridBuilder2Ptr& builder,
                                                double initialVal) {
...
size_t GridSystemData2::addAdvectableVectorData(const VectorGridBuilder2Ptr& builder,
                                                const Vector2D& initialVal) {
...
const ScalarGrid2Ptr& GridSystemData2::advectableScalarDataAt(size_t idx) const {
  return _advectableScalarDataList[idx];
}

const VectorGrid2Ptr& GridSystemData2::advectableVectorDataAt(size_t idx) const {
...
void GridSystemData2::serialize(std::vector<uint8_t>* buffer) const {
  ...
  serializeGrid(&builder, _scalarDataList, fbs::CreateScalarGridSerialized2, &scalarDataList);
  serializeGrid(&builder, _vectorDataList, fbs::CreateVectorGridSerialized2, &vectorDataList);
  serializeGrid(&builder, _advectableScalarDataList, fbs::CreateScalarGridSerialized2,
                &advScalarDataList);
  serializeGrid(&builder, _advectableVectorDataList, fbs::CreateVectorGridSerialized2,
                &advVectorDataList);

  auto gsd = fbs::CreateGridSystemData2(
      builder, &resolution, &gridSpacing, &origin, _velocityIdx,
      builder.CreateVector(scalarDataList), builder.CreateVector(vectorDataList),
      builder.CreateVector(advScalarDataList), builder.CreateVector(advVectorDataList));
...
void GridSystemData2::deserialize(const std::vector<uint8_t>& buffer) {
  ...
  resize(fbsToJet(*gsd->resolution()), fbsToJet(*gsd->gridSpacing()), fbsToJet(*gsd->origin()));
  ...
  deserializeGrid(gsd->scalarData(), Factory::buildScalarGrid2, &_scalarDataList);
  deserializeGrid(gsd->vectorData(), Factory::buildVectorGrid2, &_vectorDataList);
  ...
  _velocity = std::dynamic_pointer_cast<FaceCenteredGrid2>(_advectableVectorDataList[_velocityIdx]);
}
