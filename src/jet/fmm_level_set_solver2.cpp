...
inline double solveQuadNearBoundary(const Array2<char>& markers, ArrayAccessor2<double> output,
                                    const Vector2D& gridSpacing, const Vector2D& invGridSpacingSqr,
  ...
  double distToBndX =
      gridSpacing.x * std::abs(output(i, j)) / (std::abs(output(i, j)) + std::abs(phiX));

  double distToBndY =
      gridSpacing.y * std::abs(output(i, j)) / (std::abs(output(i, j)) + std::abs(phiY));
...
inline double solveQuad(const Array2<char>& markers, ArrayAccessor2<double> output,
                        const Vector2D& gridSpacing, const Vector2D& invGridSpacingSqr, size_t i,
                        size_t j) {
  Size2 size = output.size();
...
void FmmLevelSetSolver2::reinitialize(const ScalarGrid2& inputSdf, double maxDistance,
  ...
  markers.parallelForEachIndex([&](size_t i, size_t j) { output(i, j) = inputSdf(i, j); });

  // Solve geometrically near the boundary
  markers.forEachIndex([&](size_t i, size_t j) {
    if (!isInsideSdf(output(i, j)) && ((i > 0 && isInsideSdf(output(i - 1, j))) ||
                                       (i + 1 < size.x && isInsideSdf(output(i + 1, j))) ||
                                       (j > 0 && isInsideSdf(output(i, j - 1))) ||
                                       (j + 1 < size.y && isInsideSdf(output(i, j + 1))))) {
      output(i, j) =
          solveQuadNearBoundary(markers, output, gridSpacing, invGridSpacingSqr, 1.0, i, j);
    } else if (isInsideSdf(output(i, j)) && ((i > 0 && !isInsideSdf(output(i - 1, j))) ||
                                             (i + 1 < size.x && !isInsideSdf(output(i + 1, j))) ||
                                             (j > 0 && !isInsideSdf(output(i, j - 1))) ||
                                             (j + 1 < size.y && !isInsideSdf(output(i, j + 1))))) {
      output(i, j) =
  ...
  for (int sign = 0; sign < 2; ++sign) {
    ...
    markers.forEachIndex([&](size_t i, size_t j) {
      if (markers(i, j) != kKnown && ((i > 0 && markers(i - 1, j) == kKnown) ||
    ...
    // Flip the sign
    markers.parallelForEachIndex([&](size_t i, size_t j) { output(i, j) = -output(i, j); });
  }
}

void FmmLevelSetSolver2::extrapolate(const ScalarGrid2& input, const ScalarField2& sdf,
  ...
  sdfGrid.parallelForEachIndex([&](size_t i, size_t j) { sdfGrid(i, j) = sdf.sample(pos(i, j)); });

  extrapolate(input.constDataAccessor(), sdfGrid.constAccessor(), input.gridSpacing(), maxDistance,
              output->dataAccessor());
}

void FmmLevelSetSolver2::extrapolate(const CollocatedVectorGrid2& input, const ScalarField2& sdf,
                                     double maxDistance, CollocatedVectorGrid2* output) {
  JET_THROW_INVALID_ARG_IF(!input.hasSameShape(*output));
...
void FmmLevelSetSolver2::extrapolate(const FaceCenteredGrid2& input, const ScalarField2& sdf,
                                     double maxDistance, FaceCenteredGrid2* output) {
  JET_THROW_INVALID_ARG_IF(!input.hasSameShape(*output));
  ...
  input.parallelForEachUIndex([&](size_t i, size_t j) { sdfAtU(i, j) = sdf.sample(uPos(i, j)); });
  ...
  input.parallelForEachVIndex([&](size_t i, size_t j) { sdfAtV(i, j) = sdf.sample(vPos(i, j)); });

  extrapolate(v, sdfAtV, gridSpacing, maxDistance, output->vAccessor());
}

void FmmLevelSetSolver2::extrapolate(const ConstArrayAccessor2<double>& input,
