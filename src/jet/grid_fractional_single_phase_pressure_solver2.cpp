...
void restrict(const Array2<float>& finer, Array2<float>* coarser) {
  ...
  parallelRangeFor(kZeroSize, n.x, kZeroSize, n.y,
...
void buildSingleSystem(FdmMatrix2* A, FdmVector2* b, const Array2<float>& fluidSdf,
                       const Array2<float>& uWeights, const Array2<float>& vWeights,
...
void buildSingleSystem(MatrixCsrD* A, VectorND* x, VectorND* b, const Array2<float>& fluidSdf,
                       const Array2<float>& uWeights, const Array2<float>& vWeights,
                       std::function<Vector2D(const Vector2D&)> boundaryVel,
                       const FaceCenteredGrid2& input) {
  const Size2 size = input.resolution();
...
void GridFractionalSinglePhasePressureSolver2::solve(
    const FaceCenteredGrid2& input, double timeIntervalInSeconds, FaceCenteredGrid2* output,
    const ScalarField2& boundarySdf, const VectorField2& boundaryVelocity,
    const ScalarField2& fluidSdf, bool useCompressed) {
...
GridFractionalSinglePhasePressureSolver2::suggestedBoundaryConditionSolver() const {
  return std::make_shared<GridFractionalBoundaryConditionSolver2>();
}

const FdmLinearSystemSolver2Ptr& GridFractionalSinglePhasePressureSolver2::linearSystemSolver()
    const {
...
void GridFractionalSinglePhasePressureSolver2::buildWeights(const FaceCenteredGrid2& input,
...
void GridFractionalSinglePhasePressureSolver2::buildSystem(const FaceCenteredGrid2& input,
  ...
  if (_mgSystemSolver == nullptr) {
    ...
    FdmMgUtils2::resizeArrayWithFinest(size, maxLevels, &_mgSystem.A.levels);
    FdmMgUtils2::resizeArrayWithFinest(size, maxLevels, &_mgSystem.x.levels);
    FdmMgUtils2::resizeArrayWithFinest(size, maxLevels, &_mgSystem.b.levels);
  ...
  if (_mgSystemSolver == nullptr) {
    if (useCompressed) {
      buildSingleSystem(&_compSystem.A, &_compSystem.x, &_compSystem.b, _fluidSdf[0], _uWeights[0],
                        _vWeights[0], _boundaryVel, *finer);
    } else {
      buildSingleSystem(&_system.A, &_system.b, _fluidSdf[0], _uWeights[0], _vWeights[0],
                        _boundaryVel, *finer);
    }
  } else {
    buildSingleSystem(&_mgSystem.A.levels.front(), &_mgSystem.b.levels.front(), _fluidSdf[0],
  ...
  for (size_t l = 1; l < numLevels; ++l) {
    ...
    buildSingleSystem(&_mgSystem.A.levels[l], &_mgSystem.b.levels[l], _fluidSdf[l], _uWeights[l],
                      _vWeights[l], _boundaryVel, coarser);

    finer = &coarser;
  }
}

void GridFractionalSinglePhasePressureSolver2::applyPressureGradient(const FaceCenteredGrid2& input,
