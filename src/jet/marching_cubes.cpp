...
inline Vector3D grad(const ConstArrayAccessor3<double>& grid, ssize_t i, ssize_t j, ssize_t k,
...
// i        i+1         2i   2i+1  2i+2
//
inline size_t globalEdgeId(size_t i, size_t j, size_t k, const Size3& dim, size_t localEdgeId) {
  // See edgeConnection in marching_cubes_table.h for the edge ordering.
  static const int edgeOffset3D[12][3] = {{1, 0, 0}, {2, 0, 1}, {1, 0, 2}, {0, 0, 1},
...
// i        i+1         2i   2i+1  2i+2
//
inline size_t globalVertexId(size_t i, size_t j, size_t k, const Size3& dim, size_t localVertexId) {
  // See edgeConnection in marching_cubes_table.h for the edge ordering.
...
static void singleSquare(const std::array<double, 4>& data,
                         const std::array<size_t, 8>& vertAndEdgeIds, const Vector3D& normal,
                         const std::array<Vector3D, 4>& corners, MarchingCubeVertexMap* vertexMap,
                         TriangleMesh3* mesh, double isoValue) {
...
void marchingCubes(const ConstArrayAccessor3<double>& grid, const Vector3D& gridSize,
                   const Vector3D& origin, TriangleMesh3* mesh, double isoValue, int bndClose,
                   int bndConnectivity) {
      ...
      for (ssize_t i = 0; i < dimx - 1; ++i) {
        ...
        if (bndClose & kDirectionBack) {
          ...
          singleSquare(data, vertexAndEdgeIds, normal, corners,
                       (bndConnectivity & kDirectionBack) ? &vertexMap : &vertexMapBack, mesh,
                       isoValue);
        }
        ...
        if (bndClose & kDirectionFront) {
          ...
          singleSquare(data, vertexAndEdgeIds, normal, corners,
                       (bndConnectivity & kDirectionFront) ? &vertexMap : &vertexMapFront, mesh,
                       isoValue);
        }
      }  // i
    }    // j
    ...
    for (ssize_t k = 0; k < dimz - 1; ++k) {
      for (ssize_t j = 0; j < dimy - 1; ++j) {
        ...
        if (bndClose & kDirectionLeft) {
          ...
          singleSquare(data, vertexAndEdgeIds, normal, corners,
                       (bndConnectivity & kDirectionLeft) ? &vertexMap : &vertexMapLeft, mesh,
                       isoValue);
        }
        ...
        if (bndClose & kDirectionRight) {
          ...
          singleSquare(data, vertexAndEdgeIds, normal, corners,
                       (bndConnectivity & kDirectionRight) ? &vertexMap : &vertexMapRight, mesh,
                       isoValue);
        }
      }  // j
    }    // k
    ...
    for (ssize_t k = 0; k < dimz - 1; ++k) {
      for (ssize_t i = 0; i < dimx - 1; ++i) {
        ...
        if (bndClose & kDirectionDown) {
          ...
          singleSquare(data, vertexAndEdgeIds, normal, corners,
                       (bndConnectivity & kDirectionDown) ? &vertexMap : &vertexMapDown, mesh,
                       isoValue);
        }
        ...
        if (bndClose & kDirectionUp) {
          ...
          singleSquare(data, vertexAndEdgeIds, normal, corners,
                       (bndConnectivity & kDirectionUp) ? &vertexMap : &vertexMapUp, mesh,
                       isoValue);
        }
      }  // i
    }    // k
...
void marchingSquares(const ConstArrayAccessor2<double>& grid, const Vector2D& gridSize,
                     const Vector2D& origin, TriangleMesh3* mesh, double isoValue) {
  MarchingCubeVertexMap vertexMap;

  const Size3 dim = {grid.size(), 0};

  auto pos = [origin, gridSize](ssize_t i, ssize_t j, ssize_t k) {
    return Vector3D{origin + gridSize * Vector2D({i, j}), 0.0};
  };
