...
struct WindingNumberGatherData {
  ...
  WindingNumberGatherData operator+(const WindingNumberGatherData& other) const {
...
WindingNumberGatherData postOrderTraversal(const Bvh3<size_t>& bvh, size_t nodeIndex,
                                           const GatherFunc& visitorFunc,
  ...
  if (bvh.isLeaf(nodeIndex)) {
    ...
    data = data + postOrderTraversal(bvh, children.first, visitorFunc, leafFunc, initGatherData);
    data = data + postOrderTraversal(bvh, children.second, visitorFunc, leafFunc, initGatherData);
  }
...
const Point3UI& TriangleMesh3::pointIndex(size_t i) const { return _pointIndices[i]; }

Point3UI& TriangleMesh3::pointIndex(size_t i) { return _pointIndices[i]; }

const Point3UI& TriangleMesh3::normalIndex(size_t i) const { return _normalIndices[i]; }
...
void TriangleMesh3::scale(double factor) {
  parallelFor(kZeroSize, numberOfPoints(), [this, factor](size_t i) { _points[i] *= factor; });
  invalidateCache();
}

void TriangleMesh3::translate(const Vector3D& t) {
  parallelFor(kZeroSize, numberOfPoints(), [this, t](size_t i) { _points[i] += t; });
  invalidateCache();
}

void TriangleMesh3::rotate(const Quaternion<double>& q) {
  parallelFor(kZeroSize, numberOfPoints(), [this, q](size_t i) { _points[i] = q * _points[i]; });

  parallelFor(kZeroSize, numberOfNormals(), [this, q](size_t i) { _normals[i] = q * _normals[i]; });
    ...
    for (size_t f = 0; f < shape.mesh.num_face_vertices.size(); ++f) {
      ...
      if (fv == 3) {
        if (!attrib.vertices.empty()) {
          addPointTriangle({shape.mesh.indices[idx].vertex_index,
                            shape.mesh.indices[idx + 1].vertex_index,
                            shape.mesh.indices[idx + 2].vertex_index});
        }

        if (!attrib.normals.empty()) {
          addNormalTriangle({shape.mesh.indices[idx].normal_index,
...
void TriangleMesh3::buildWindingNumbers() const {
  ...
  if (_wnInvalidated) {
    ...
    const auto visitorFunc = [&](size_t nodeIndex, const WindingNumberGatherData& data) {
      _wnAreaWeightedNormalSums[nodeIndex] = data.areaWeightedNormalSums;
      _wnAreaWeightedAvgPositions[nodeIndex] = data.areaWeightedPositionSums / data.areaSums;
    };
    ...
    postOrderTraversal(_bvh, 0, visitorFunc, leafFunc, WindingNumberGatherData());

    _wnInvalidated = false;
  }
}

double TriangleMesh3::windingNumber(const Vector3D& queryPoint, size_t triIndex) const {
  ...
  const double denom = a * b * c + va.dot(vb) * c + vb.dot(vc) * a + vc.dot(va) * b;
...
double TriangleMesh3::fastWindingNumber(const Vector3D& queryPoint, double accuracy) const {
  buildWindingNumbers();
...
TriangleMesh3 TriangleMesh3::Builder::build() const {
  return TriangleMesh3(_points, _normals, _uvs, _pointIndices, _normalIndices, _uvIndices,
                       _transform, _isNormalFlipped);
}

TriangleMesh3Ptr TriangleMesh3::Builder::makeShared() const {
  return std::shared_ptr<TriangleMesh3>(
      new TriangleMesh3(_points, _normals, _uvs, _pointIndices, _normalIndices, _uvIndices,
                        _transform, _isNormalFlipped),
