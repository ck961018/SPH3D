...
static const size_t kDefaultHashGridResolution = 8;
...
ArrayAccessor1<Vector3D> ParticleSystemData3::positions() { return vectorDataAt(_positionIdx); }
...
void ParticleSystemData3::addParticle(const Vector3D& newPosition, const Vector3D& newVelocity,
...
void ParticleSystemData3::buildNeighborLists(double maxSearchRadius) {
  ...
  JET_INFO << "Building neighbor list took: " << timer.durationInSeconds() << " seconds";
}
...
void ParticleSystemData3::serializeParticleSystemData(
  ...
  auto neighborSearcherType = builder->CreateString(_neighborSearcher->typeName());
  ...
  auto fbsNeighborSearcher = fbs::CreatePointNeighborSearcherSerialized3(
      *builder, neighborSearcherType,
      builder->CreateVector(neighborSearcherSerialized.data(), neighborSearcherSerialized.size()));
  ...
  for (const auto& neighbors : _neighborLists) {
    ...
    flatbuffers::Offset<fbs::ParticleNeighborList3> fbsNeighborList =
        fbs::CreateParticleNeighborList3(
            *builder, builder->CreateVector(neighbors64.data(), neighbors64.size()));
  ...
  *fbsParticleSystemData = fbs::CreateParticleSystemData3(
      *builder, _radius, _mass, _positionIdx, _velocityIdx, _forceIdx, fbsScalarDataList,
...
void ParticleSystemData3::deserializeParticleSystemData(
  ...
  _neighborSearcher = Factory::buildPointNeighborSearcher3(fbsNeighborSearcher->type()->c_str());
  std::vector<uint8_t> neighborSearcherSerialized(fbsNeighborSearcher->data()->begin(),
  ...
  for (uint32_t i = 0; i < fbsNeighborLists->size(); ++i) {
    ...
    std::transform(fbsNeighborList->data()->begin(), fbsNeighborList->data()->end(),
